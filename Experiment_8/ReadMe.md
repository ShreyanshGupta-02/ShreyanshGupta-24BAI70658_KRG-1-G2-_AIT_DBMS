...
Analyze the output.
Procedure
Open Oracle SQL Developer or pgAdmin and connect to the database.
Create the employee table with fields such as id, name, and gender.
Insert sample records into the table.
Create a PL/SQL stored procedure with IN and OUT parameters.
Write a SELECT COUNT(*) query inside the procedure.
Compile the procedure successfully.
Execute the procedure using different gender values.
Display the output.
